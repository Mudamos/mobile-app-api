...
class Vote {
  ...
  static findByUserId(plipId, userId) {
    var query = 'SELECT * FROM vote WHERE UserId= ? AND PetitionId = ?';
    return new Promise((resolve, reject) => {
      var connection = new Connection();
      connection.getConnection(function (err) {
        if (err) {
        ...
        conn.query(query, [userId, plipId], function (error, rows, fields) {
          conn.release();
          resolve(_.first(rows));
        });
      })
  ...
  static findByIdInfo(plipId, { cityName, uf } = {}) {
    ...
    if (cityName && uf) {
      ...
      replacements = [...replacements, cityName, uf];
    }
  ...
  static findAllPDF() {
    var query = 'SELECT v.Message AS vote_message,v.PetitionId AS petition_id,v.Id AS vote_id,v.Signature as link_vote_signature,p.Name AS petition_name,p.PageUrl AS petition_page_url,p.TxStamp AS petition_tx_stamp,p.DigSig AS petition_dig_sig,u.State AS user_state,u.UF AS user_uf,u.City AS user_city,u.District AS user_district FROM vote AS v JOIN user AS u ON u.VoteIdCard = v.VoteIdCard JOIN petition AS p ON p.Id = v.PetitionId AND p.Status = 1 ORDER BY u.State, u.UF, u.City, u.District collate utf8_bin, u.Name ASC';
    return new Promise((resolve, reject) => {
      var connection = new Connection();
      connection.getConnection(function (err) {
        if (err) {
        ...
        conn.query(query, function (error, rows, fields) {
          conn.release();
          _.each(rows, row => {
            row.link_vote_signature = `${config.get('LINK_SIGNATURE')}/${sign.toHex(row.link_vote_signature)}`
          })
          resolve(rows || []);
        })
      })
  ...
  static findBySignatures(signatures) {
    var query = `SELECT
                  Id AS vote_id,
