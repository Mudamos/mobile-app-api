...
class User {
  ...
  static findByEmail(email) {
    var query = `SELECT voteCity.id as voteCityId, voteCity.name as voteCityName, voteCity.uf as voteCityUf, u.Id AS user_id, u.Name AS user_name, DATE_FORMAT(u.Birthday,\'%Y-%m-%d\') AS user_birthday,u.VoteIdCard AS user_voteidcard,u.CPF AS user_cpf,u.ZipCode AS user_zipcode, u.State AS user_state, u.UF as user_uf, u.Lat as user_lat, u.Lng as user_lng, u.City as user_city , u.District  as user_district, DATE_FORMAT(u.Validate, \'%Y-%m-%dT%TZ\') AS user_validate, u.TermsAccepted AS terms_accepted, uw.Id AS user_wallet_id, uw.WalletId AS wallet_key,uw.Status AS wallet_status, m.Id AS mobile_id, m.Status AS mobile_status,m.Number AS mobile_number,m.Imei AS mobile_imei,m.Brand AS mobile_brand,m.Model AS mobile_model,m.SO AS mobile_so,m.SOVersion AS mobile_so_version,m.ScreenSize AS mobile_screensize, p.Type AS profile_type,p.Email AS profile_email,p.ProfileId AS profile_id, p.Picture AS profile_picture, p.IsAvatar AS has_saved_avatar, p.PictureUpdatedAt as picture_updated_at FROM user as u LEFT JOIN user_wallet as uw ON u.Id=uw.UserId LEFT JOIN user_mobile as um ON u.Id=um.UserId LEFT JOIN mobile as m ON m.Id=um.MobileId LEFT JOIN user_profile as up ON u.id=up.UserId LEFT JOIN profile as p ON up.ProfileId=p.Id LEFT JOIN city voteCity ON voteCity.id = u.voteCityId WHERE p.email = ? AND p.Type = 'app'  ORDER BY u.Id ,  m.Id desc , uw.Id desc LIMIT 1;`;
    return new Promise((resolve, reject) => {
      var connection = new Connection();
      connection.getConnection(function (err) {
        if (err) {
        ...
        conn.query(query, paramns, function (error, rows, fields) {
         ...
         if (rows.length) {
            _.each(rows, (row) => {
              ...
              row.profile_picture = buildUserPictureUrl(row.profile_picture, row.picture_updated_at);
            })
            resolve(rows);
          } else {
            resolve(null);
  ...
  static findByCpf(cpf) {
    const query = `SELECT voteCity.id as voteCityId, voteCity.name as voteCityName, voteCity.uf as voteCityUf, u.Id AS user_id, u.Name AS user_name, DATE_FORMAT(u.Birthday,\'%Y-%m-%d\') AS user_birthday,u.VoteIdCard AS user_voteidcard,u.CPF AS user_cpf,u.ZipCode AS user_zipcode, u.State AS user_state, u.UF as user_uf, u.Lat as user_lat, u.Lng as user_lng, u.City as user_city , u.District  as user_district, DATE_FORMAT(u.Validate, \'%Y-%m-%dT%TZ\') AS user_validate, u.TermsAccepted AS terms_accepted, uw.Id AS user_wallet_id, uw.WalletId AS wallet_key,uw.Status AS wallet_status, m.Id AS mobile_id, m.Status AS mobile_status,m.Number AS mobile_number,m.Imei AS mobile_imei,m.Brand AS mobile_brand,m.Model AS mobile_model,m.SO AS mobile_so,m.SOVersion AS mobile_so_version,m.ScreenSize AS mobile_screensize, p.Type AS profile_type,p.Email AS profile_email,p.ProfileId AS profile_id, p.Picture AS profile_picture, p.IsAvatar AS has_saved_avatar, p.PictureUpdatedAt as picture_updated_at FROM user as u LEFT JOIN user_wallet as uw ON u.Id=uw.UserId LEFT JOIN user_mobile as um ON u.Id=um.UserId LEFT JOIN mobile as m ON m.Id=um.MobileId LEFT JOIN user_profile as up ON u.id=up.UserId LEFT JOIN profile as p ON up.ProfileId=p.Id LEFT JOIN city voteCity ON voteCity.id = u.voteCityId WHERE u.cpf = ? ORDER BY m.id desc, uw.Id desc`;
    ...
    return new Promise((resolve, reject) => {
      ...
      connection.getConnection(function (err) {
        ...
        conn.query(query, [cpf], function (error, rows, fields) {
          conn.release();
  ...
  static removeAccount(id){
    ...
    return new Promise((resolve, reject) => {
      ...
      connection.getConnection(function (err) {
        ...
        conn.query(query, [id], function (error, rows, fields) {
          conn.release();
            if (!error && _.first(rows[0]).date_time) {
            resolve(true);
          }else{
            resolve(false);
          }
