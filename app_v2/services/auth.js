    ...
    return UserModel.findByEmailOrProfile(profile.emails[0].value, profile.id)
      .then(users => {
        ...
        if (!users || !users.length) {
          debug("Will create facebook user");
          ...
          debug("Will add facebook profile");
          ...
          debug("User previously logged in");
      ...
      .then(user => {
        debug("User", user);
        AccessModel.access('SIGN-FACEBOOK', user.user_id, token.access_token, 0);

        debug("Will handle auth token");
